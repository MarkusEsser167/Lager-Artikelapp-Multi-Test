...
import { loadArtikelListe, loadMitarbeiterListe, loadArtikelLagerplatzListe, getCurrentStandort } from '../refData.js';
...
export async function renderNewFehlbestand(container, router) {
  ...
  header.innerHTML = `<h1>Fehlbestand melden</h1><p class="view-subtitle">Lädt…</p>`;
  ...
  const [artikelListe, mitarbeiterListe, artikelLagerplatzListe, standort] = await Promise.all([
    loadArtikelListe(),
  ...
  loading.remove();
  header.querySelector('.view-subtitle').textContent =
    `Artikel per Mail an ${standort ? standort.zentraleMail : '–'} melden, ohne Lagerplatz oder Menge.`;
